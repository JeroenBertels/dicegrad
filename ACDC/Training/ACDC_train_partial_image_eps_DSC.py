...
def train(run_name, experiment_name, fold_i=0, base_dir = None,reduce_along_batch = False,reduce_along_features =False,removeValES = False,removeValMyoES = True, ED_fraction = 0.5,eps_corr = 1,onlyPositives = False,noEpsLVRV=False):
    nb_classes = 4

    train_data = Mirc()
    train_data.add(create_dataset_ACDC_removeMyoES(os.path.join(main_dir,'Preprocessed_images'),"train", fold_i=fold_i,ED_fraction=ED_fraction))
    val_data = Mirc()
    ...
    soft_dice = get_loss("dice_loss",reduce_along_batch=reduce_along_batch,reduce_along_features=reduce_along_features,weights=[0,1,1,1],eps=eps_estimate) # can work with multiclass, but also takes into account first dimension (background)
...
if __name__ == '__main__':
    for f in range(0,5):
        train('Trained_models','partial_image_eps_DSC', fold_i=f,
              base_dir=main_dir, reduce_along_batch=False, ED_fraction=0.75)
