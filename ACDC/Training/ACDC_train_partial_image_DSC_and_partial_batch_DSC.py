...
def train(run_name, experiment_name, fold_i=0, base_dir = None,reduce_along_batch = False,reduce_along_features=False,removeValES = False,removeValMyoES = True):
    nb_classes = 4

    train_data = Mirc()
    train_data.add(create_dataset_ACDC_removeMyoES(os.path.join(main_dir,'Preprocessed_images'),"train", fold_i=fold_i))
    val_data = Mirc()
    val_data.add(create_dataset_ACDC_removeMyoES(os.path.join(main_dir,'Preprocessed_images'),"val",fold_i=fold_i,removeES=removeValES,removeMyoES = removeValMyoES))
    ...
    x_path, y_path = GridCrop(x_path, (192,192,48), n=1, nonzero=True)(x_path, y_path)
    ...
    # if you want to have unique transformer names for your entire network, you can first make one large creator and afterwards make the individual creators
    ...
    soft_dice = get_loss("dice_loss", reduce_along_batch=reduce_along_batch,reduce_along_features=reduce_along_features, weights=[0,1,1,1])
    ...
    callbacks = [
        DvnModelEvaluator(dvn_model, "full_val", val_sampler, freq=30,
                          output_dirs=output_structure.val_images_output_dirs),
        # watch out, here you do need to make sure that the order of the val_sampler (shuffle=False by default) is te same as the order of your output_dirs...
        DvnModelCheckpoint(dvn_model, output_structure.models_dir, freq=30),
        # every 30 epochs the model will be saved (for e.g. parallel offline use for testing some things)
        ReduceLROnPlateau(monitor="val__loss", factor=0.2, patience=40, mode="min", min_delta=0.0001),
        EarlyStopping(monitor="val__loss", patience=80, mode="min", min_delta=0.0001)
    ]
...
if __name__ == '__main__':
    for f in range(0,5):
        train('Trained_models', 'partial_image_DSC', fold_i=f,
              base_dir=main_dir, reduce_along_batch=False)
        tf.keras.backend.clear_session()

    for f in range(0, 5):
        train('Trained_models', 'partial_batch_DSC', fold_i=f,
              base_dir=main_dir, reduce_along_batch=True)
        tf.keras.backend.clear_session()
