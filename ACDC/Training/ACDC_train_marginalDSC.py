...
from losses_missing_labels import get_loss
...
def train(run_name, experiment_name, fold_i=0, base_dir = None, removeValES = False,removeValMyoES = True,loss="marginal_dice_loss", metric="marginal_dice_metric"):
    nb_classes = 4
    ...
    soft_dice = get_loss(loss)
    dice_score = get_metric(metric)
...
if __name__ == '__main__':
    for f in range(0,5):
        train('Trained_models', 'partial_marginal_DSC', fold_i=f,
              base_dir=main_dir, loss="marginal_dice_loss", metric="marginal_dice_metric")
