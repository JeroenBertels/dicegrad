...
def main(run_name, experiment_name, fold_i=0, batch_size=1, fraction=1):
    # load datasets
    train_data = Mirc(create_dataset("train", fold_i=fold_i, fraction=fraction))
    val_data = Mirc(create_dataset("val", fold_i=fold_i))
    ...
    x_path, y_path = GeometricCrop(y_input, keras_model.output_shape[1:4])(x_path, y_path)
    ...
    x_path, y_path = GeometricCrop(y_input, keras_model.output_shape[1:4])(x_input, y_input)
    ...
    soft_dice = get_loss("dice_loss", reduce_along_batch=False, reduce_along_features=False, eps=12412)
    ...
    # callbacks = [
    #     DvnModelEvaluator(dvn_model, "full_val", val_sampler, freq=10, output_dirs=output_structure.val_images_output_dirs),
    ...
    dvn_model = DvnModel.load_model("/usr/local/micapollo01/MIC/DATA/STAFF/jberte3/tmp/datasets/Runs/brats_hgg_x/Experiments/perimage-eps_auto-1e-3-30_bs-8_f-100/Round_0/Fold_0/dvn_model_final")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='brats_benchmark_perimage_eps')
